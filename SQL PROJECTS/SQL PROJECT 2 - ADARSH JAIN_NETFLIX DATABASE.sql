...
SELECT * FROM viewinghistory 
ORDER BY Runtime DESC LIMIT 3;

-- 2. Partition the viewing hours by category and genre 
-- to find the top genre in each category. Use the rank function to rank genres within each category. 

SELECT c.contentID,
c.genre,
c.category , 
vh.runtime,
RANK() OVER(PARTITION BY c.genre  ORDER BY runtime DESC) AS `genre_rank`
FROM content c
JOIN viewinghistory vh ON c.contentID = vh.contentID;
...
SELECT p.PlanID,p.PlanName,COUNT(SUB.CustID)AS subscriber_count
FROM subscribes SUB
JOIN plans p ON SUB.PlanID=p.PlanID
GROUP BY p.PlanID,p.PlanName
ORDER BY COUNT(SUB.CustID) DESC;

-- 4.Which device type is most commonly used to access Netflix content? Provide the Device 
-- Type and count of accesses.

SELECT D.DeviceType, COUNT(U.ProfileID) AS count_of_acess
FROM devices D
JOIN uses U ON D.DeviceID = U.DeviceID
GROUP BY D.DeviceType
ORDER BY count_of_acess DESC LIMIT 1;
...
ROUND(AVG(vh.Runtime) OVER(PARTITION BY c.category),2) AS average_runtime
FROM content c
JOIN viewinghistory vh ON c.contentID = vh.ContentID; 
...
SELECT CLP.Language, COUNT(C.CUSTID) AS `no_of_customers`
FROM customerslanguagepreferred CLP
JOIN customers C ON CLP.CustID =C.CUSTID
GROUP BY CLP.Language
...
SELECT (SELECT COUNT(CustID) FROM adultacc ad
JOIN profiles P ON ad.ProfileID = P.ProfileID) AS total_customers_Of_adults_account,
(SELECT COUNT(CustID) FROM childacc ch
JOIN profiles P ON ch.ProfileID = P.ProfileID) AS total_customers_of_childs_account;


-- 8. Determine the average number of profiles created per customer account. 
WITH n_t AS
(SELECT C.CUSTID, COUNT(P.ProfileID) AS no_of_profile_created 
FROM customers C 
JOIN profiles P ON C.CUSTID = P.CustID
GROUP BY C.CUSTID)
SELECT ROUND(SUM(no_of_profile_created)/COUNT(CUSTID),2) AS avg_profiles_per_acc
FROM n_t; 
...
SELECT C.TitleName,ROUND(AVG(Runtime),2) AS avg_viewing_time
FROM content C 
JOIN viewinghistory vh ON C.ContentID = vh.ContentID
...
SELECT * FROM content;
SELECT Category,COUNT(ContentID) AS content_of_category
FROM content c 
GROUP BY category;

SELECT Genre,COUNT(ContentID) AS content_of_ 
FROM content C
GROUP BY Genre;

-- 11. Compare the number of customers that have unlimited access and who do not.
SELECT DISTINCT P.ContentAccess, 
COUNT(S.CustID) OVER(PARTITION BY P.ContentAccess) AS no_of_subscribers
FROM plans P
JOIN subscribes S ON P.PlanID = S.PlanID
ORDER BY no_of_subscribers DESC; 

-- 12. Find Average monthly price for plans with Content Access as "unlimited".
SELECT * FROM plans;
SELECT ContentAccess,ROUND(AVG(MonthlyPrice),2)AS avg_price
FROM plans
...
SELECT C.CUSTID,C.FNAME,S.EndDate
FROM customers C
JOIN subscribes S ON C.CUSTID = S.CustID
WHERE YEAR(EndDate)< 2028
ORDER BY S.EndDate DESC ,C.FNAME DESC; 

-- 14. Display Average Revenue generated from each city. Rank city based on average revenue.

WITH city_revenue_table AS
(SELECT PM.City, SUM(P.MonthlyPrice) AS average_revenue
FROM paymentmethod PM
JOIN subscribes S ON PM.CUSTID = S.CustID
JOIN plans P ON S.PlanID = P.PlanID
GROUP BY PM.City)
 SELECT *, DENSE_RANK() OVER( ORDER BY average_revenue DESC) AS city_rank_on_revenue
 FROM city_revenue_table;
...
WITH n_t AS
(
SELECT C.Genre,C.Category, COUNT(VH.ProfileID) AS 'Count_of_Views',
RANK() OVER(PARTITION BY C.Category  ORDER BY COUNT(VH.ProfileID) DESC) 'rank_over_most_veiws'
FROM content C 
JOIN viewinghistory VH ON C.ContentID = VH.ContentID
JOIN adultacc A ON VH.ProfileID = A.ProfileID
GROUP BY C.Genre,C.Category)
SELECT Genre,Category,rank_over_most_veiws
FROM n_t
WHERE rank_over_most_veiws=1;