...
SELECT customer_name, COUNT(order_id) AS `highest_orders` FROM order_detail OD
...
-- 2. Find the restaurant that has received the highest average rating.
 ...
 FROM restaurant ORDER BY rrating DESC LIMIT 1; 
 
 SELECT * FROM restaurant;
 
 --  3. List all orders from order_detail that were delivered in under 30 minutes.
SELECT * FROM  order_detail WHERE TIMESTAMPDIFF( MINUTE, order_time,delivered_time )< 30;

SELECT * FROM  order_detail WHERE delivered_time <= order_time + INTERVAL 30 MINUTE; 
...
SELECT f.food_id,food_name ,SUM( FO.quantity * f.price_per_unit) AS total_revenue
FROM foods f 
JOIN order_food FO ON f.food_id = FO.food_id 
GROUP BY f.food_name,f.food_id ORDER BY total_revenue DESC ; 
...
SELECT r.restaurant_name,
  SUM( FO.quantity * f.price_per_unit) AS total_revenue
FROM foods f
JOIN order_food FO ON f.food_id = FO.food_id 
...
ORDER BY total_revenue DESC LIMIT 1 OFFSET 1; 
 ...
 fo.quantity
 FROM order_food fo
 INNER JOIN foods ON foods.food_id = fo.food_id
 GROUP BY foods.food_name, fo.food_id,fo.quantity
...
SELECT employee_name, employee_id, AVG(employee_avg_rating) AS `higst_avg_delivery_rating` FROM zomato_employee
GROUP BY employee_name,employee_id
ORDER BY `higst_avg_delivery_rating` DESC LIMIT 3;

--- 8) Determine the month with the highest number of total orders placed.
SELECT * FROM order_detail;

SELECT 
MONTH(order_time) as `order month`,
COUNT(*) AS `total orders`
FROM order_detail
GROUP BY MONTH(order_time) ORDER BY `total orders` DESC LIMIT 1;
 
 
--- 9) Calculate the average order amount for each customer. Order by average order amount in descending order

SELECT customer_id,customer_name FROM  order_food ;
SELECT * FROM customer;
SELECT * FROM order_food;
SELECT * FROM foods;
...
customer_name,
AVG(FO.quantity * f.price_per_unit) AS `avg order amount`
FROM foods f 
JOIN order_food FO ON f.food_id = FO.food_id 
JOIN customer c ON FO.customer_id = c.customer_id
GROUP BY c.customer_id,customer_name ORDER BY `avg order amount` DESC; 
...
--- 10.Identify the most frequent customer for each restaurant.
 ...
 WHERE od.restaurant_id = r.restaurant_id
 GROUP BY c.customer_name
 ORDER BY COUNT(*) DESC
 LIMIT 1 
 ) AS most_frequent_customer FROM restaurant r;
...
SELECT COUNT(order_time) AS `weekend orders` FROM order_detail 
...
---- 12.Calculate the average delivery time (in minutes) for orders placed on weekdays versus weekends.
 ...
 SELECT
(CASE WHEN DAYOFWEEK(delivered_time) IN (1,7) THEN "weekend" 
ELSE "weekday"
END) AS `day type`,
ROUND(AVG(TIMESTAMPDIFF(MINUTE,order_time,delivered_time))) AS `avg delivered time`
FROM order_detail 
GROUP BY `day type`;
...
SELECT * FROM order_detail;
--- 13.
 /* List the top 5 most expensive food items. 
Display food_id, food_name, and price_per_unit for all food items with the highest 5 prices. */ 

WITH top_5 AS 
     ( SELECT *, DENSE_RANK() OVER(ORDER BY price_per_unit DESC) AS items 
     FROM foods 
     )
     SELECT * FROM top_5
     WHERE items <= 5; 

SELECT * FROM foods ORDER BY price_per_unit DESC LIMIT 5;

--- 14.Find the restaurant with the most diverse menu (i.e., the highest number of food items).

SELECT r.restaurant_id,r.restaurant_name, COUNT(FO.food_id) AS `no of food item` FROM restaurant r 
JOIN order_food FO ON r.restaurant_id = FO.restaurant_id
...
SUM(f.price_per_unit * FO.quantity) AS ` total payment amount`
FROM payment_table pt 
JOIN order_food FO ON pt.order_id = FO.order_id
